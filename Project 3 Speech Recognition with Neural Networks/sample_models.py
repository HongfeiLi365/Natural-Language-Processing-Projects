...
def final_model(input_dim, units, recur_layers, output_dim=29):
    ...
    x = GRU(units, activation='relu',
            return_sequences=True, implementation=2,dropout=0.1, name='rnn_0')(input_data)
    x = BatchNormalization()(x)
    for i in range(1,recur_layers):
        x = GRU(units, activation='relu',
                return_sequences=True, implementation=2,dropout=0.1,name='rnn_'+str(i))(x)
        x = BatchNormalization()(x)
